...
const pageId = process.env.PAGE_ID || resources;
const lightningStylesheets = process.env.LIGHTNING_STYLESHEETS || false;
const showHeader = process.env.SHOWHEADER || false;
const sideBar = process.env.SIDEBAR || false;
...
const otherPageAttrs = `sidebar="false" id="${pageId}" lightningStylesheets="${lightningStylesheets}" showHeader="${showHeader}" sideBar="${sideBar}" docType="html-5.0"`;
...
gulp.task('deploy', function (success, error) {
  console.log('***************** package zip and deploy task ******************** ');
...
gulp.task('default', function() {
  console.log('Default empty task');
});
